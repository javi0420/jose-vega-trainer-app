-- Fix CRITICAL: COALESCE types uuid and boolean cannot be matched
-- The column 'created_by_trainer' is UUID (trainer ID), 'assigned_from_trainer' is BOOLEAN
-- We need to return a BOOLEAN indicating if routine was created/assigned by a trainer
    ...
    RETURN (
        ...
        SELECT COALESCE(jsonb_agg(
            jsonb_build_object(
                ...
                'user_id', r.user_id,
                -- FIXED: Return boolean indicating if created/assigned by trainer
                -- Use boolean OR instead of COALESCE to avoid type mismatch
